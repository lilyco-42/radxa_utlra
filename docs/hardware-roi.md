...
- **已接入 VP 短视频渲染管线**(`adapter-hyperframes/dist/render.js`)：
  Cedar 优先 + `libx264 veryfast` 回退，15s 超时 + pgrep 守卫
- 限制：90°/270° 旋转上游禁用(用软件旋转预处理)；H.265 未测
- **坑**：`aw-h264-encoder` 默认 Level 3.1 不能编 1080p@60（输出 0 字节 + D 状态挂死，
  SIGKILL 无效需重启板子）；1080p 必须设 `--level 40` 以上
